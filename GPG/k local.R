...
data$k_jbstatp = ifelse(data$k_jbstat==2, 1,0) #in ft or pt emp
data$k_jbstatf = ifelse(data$k_jbstat==12&13, 1,0) #furlough/temp laid off
data$k_jbsizes = ifelse(data$k_jbsize==1&2&3&4, 1,0) #small firm 
data$k_jbsizel = ifelse(data$k_jbsize==8&9, 1,0) #large firm
...
data$k_urban_dv <- ifelse(data$k_urban_dv==1, 1, 0) #urban
...
data$k_mhealthtypn1 <- ifelse(data$k_mhealthtypn1==1,1,0) #has anxiety 
...
data$k_hedlik <- ifelse(data$k_hedlik==3&4&5,1,0) #feels they were not likely to purs FE
data$k_qfhigh_dvd <- ifelse(data$k_qfhigh_dv == 1&2&3&4&5&6, 1,0) #degree educated
data$k_qfhigh_dva <- ifelse(data$k_qfhigh_dv == 7&8&9&10&11, 1,0) #a level educatrd
data$k_qfhigh_dvn <- ifelse(data$k_qfhigh_dv == 96, 1,0) #no formal quals
...
data$k_sasian <- ifelse(data$k_racel ==9 & 10 &11,1,0) #south asian
data$k_carr <- ifelse(data$k_racel ==14,1,0) #carribean
data$k_afr <- ifelse(data$k_racel ==15,1,0) #african
