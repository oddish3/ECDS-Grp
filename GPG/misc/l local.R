...
data$l_jbstatp = ifelse(data$l_jbstat==2, 1,0) #in ft or pt emp
data$l_jbstatf = ifelse(data$l_jbstat==12&13, 1,0) #furlough/temp laid off
data$l_jbsizes = ifelse(data$l_jbsize==1&2&3&4, 1,0) #small firm 
data$l_jbsizel = ifelse(data$l_jbsize==8&9, 1,0) #large firm
data$l_tujbpl = ifelse(data$l_tujbpl==1, 1,0) #in a union
...
data$l_urban_dv <- ifelse(data$l_urban_dv==1, 1, 0) #urban
...
data$l_mhealthtypn1 <- ifelse(data$l_mhealthtypn1==1,1,0) #has anxiety 
...
data$l_hedlik <- ifelse(data$l_hedlik==3&4&5,1,0) #feels they were not likely to purs FE
data$l_qfhigh_dvd <- ifelse(data$l_qfhigh_dv == 1&2&3&4&5&6, 1,0) #degree educated
data$l_qfhigh_dva <- ifelse(data$l_qfhigh_dv == 7&8&9&10&11, 1,0) #a level educatrd
data$l_qfhigh_dvn <- ifelse(data$l_qfhigh_dv == 96, 1,0) #no formal quals
...
data$l_sasian <- ifelse(data$l_racel ==9 & 10 &11,1,0) #south asian
data$l_carr <- ifelse(data$l_racel ==14,1,0) #carribean
data$l_afr <- ifelse(data$l_racel ==15,1,0) #african
